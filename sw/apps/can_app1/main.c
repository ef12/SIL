...
#define MY_PORT          7401U
#define PEER_PORT        7402U
#define MY_MSG_ID        0x18FF50E5U
...
int main(void)
{
  ...
  printf("[App1] Starting on port %u\n", MY_PORT);

  if (!udp_socket_init(&socket, MY_PORT))
  {
    printf("[App1] Failed to init socket\n");
    return 1;
  }

  if (!can_transport_udp_init(&transport, &socket, "127.0.0.1", PEER_PORT))
  {
    printf("[App1] Failed to init transport\n");
  ...
  {
    printf("[App1] Failed to set receive timeout\n");
    udp_socket_close(&socket);
    return 1;
  }
  ...
  while (1)
  {
    ...
    tx_frame.dlc = 3U;
    tx_frame.data[0] = 0xCAU;
    tx_frame.data[1] = 0xFEU;
    tx_frame.data[2] = counter;

    printf("[App1] TX -> ID=0x%08X DLC=%u Data=[%02X %02X %02X]\n", tx_frame.id, tx_frame.dlc,
    ...
    for (uint32_t i = 0U; i < RX_LOOPS; i++)
    {
      ...
      if (can_transport_udp_receive_frame(&transport, &rx_frame, sender_ip, sizeof(sender_ip),
                                          &sender_port))
      {
        printf("[App1] RX <- ID=0x%08X DLC=%u Data=[%02X %02X %02X] from %s:%u\n", rx_frame.id,
        ...
        if (!IS_ECHO(rx_frame.id))
        {
          ...
          printf("[App1] ECHO -> ID=0x%08X DLC=%u Data=[%02X %02X %02X]\n", echo_frame.id,
