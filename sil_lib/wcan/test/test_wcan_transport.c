...
static int expect_frame(wcan_shm_socket_t *socket, uint32_t identifier)
{
    wcan_frame_t frame;
    int status = wcan_shm_recv_timeout(socket, &frame, 2000);
...
static int expect_silence(wcan_shm_socket_t *socket)
{
    wcan_frame_t frame;
    int status = wcan_shm_recv_timeout(socket, &frame, 150);
...
static int run_crash_child(const char *bus_name)
{
    ...
    if (wcan_shm_open(&socket, bus_name, 0) != WCAN_OK) {
        return 1;
    }
    (void)wcan_shm_node_index(&socket, &index);
...
{
    wcan_shm_socket_t keeper = WCAN_SHM_SOCKET_INITIALIZER;
    wcan_shm_socket_t reclaimed = WCAN_SHM_SOCKET_INITIALIZER;
    ...
    CHECK(wcan_shm_open(&keeper, "shmcrash", 0) == WCAN_OK);
    CHECK(wcan_shm_node_count(&keeper, &count_before) == WCAN_OK);
    ...
    CHECK(wcan_shm_node_count(&keeper, &count_stale) == WCAN_OK);
    CHECK(count_stale == count_before + 1u);

    /* Opening again must reclaim the dead slot rather than consume a new one. */
    CHECK(wcan_shm_open(&reclaimed, "shmcrash", 0) == WCAN_OK);
    CHECK(wcan_shm_node_index(&reclaimed, &index) == WCAN_OK);
    CHECK(wcan_shm_node_count(&reclaimed, &count_after) == WCAN_OK);
    CHECK(count_after == count_stale);

    CHECK(wcan_shm_close(&reclaimed) == WCAN_OK);
    CHECK(wcan_shm_close(&keeper) == WCAN_OK);
...
int main(int argc, char **argv)
{
    ...
    wcan_shm_socket_t echoed = WCAN_SHM_SOCKET_INITIALIZER;
    wcan_shm_socket_t isolated = WCAN_SHM_SOCKET_INITIALIZER;
    wcan_frame_t frame;
    ...
    CHECK(wcan_shm_open(&a, "shm0", 0) == WCAN_OK);
    CHECK(wcan_shm_open(&b, "shm0", 0) == WCAN_OK);
    CHECK(wcan_shm_open(&c, "shm0", 0) == WCAN_OK);
    CHECK(wcan_shm_open(&isolated, "shm1", 0) == WCAN_OK);
    ...
    CHECK(wcan_shm_open(&echoed, "shm0", WCAN_SHM_OPEN_ECHO) == WCAN_OK);
    make_frame(&frame, 0x123u);
    frame.flags = 0;
    CHECK(wcan_shm_send(&echoed, &frame) == WCAN_OK);
    ...
    CHECK(wcan_shm_close(&echoed) == WCAN_OK);
    CHECK(wcan_shm_close(&isolated) == WCAN_OK);
    ...
    puts("wcan_shm_test: all checks passed");
    return 0;
}
